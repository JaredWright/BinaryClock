...
uint8_t ds3231_set_mode(uint8_t mode)
{
	...
	return avr_i2c_write(0x00 /* ?? */, 0x2, &value, 1);
}
...
{
	if(!t) return 1;
	return avr_i2c_write(0x00 /* ?? */, 0, (uint8_t *)t, sizeof(struct ds3231_time));
}
...
{
	if(!t) return 1;
	return avr_i2c_read(0x00 /* ?? */, 0, (uint8_t *)t, sizeof(struct ds3231_time)); 
}
